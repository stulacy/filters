...
// [[Rcpp::export]]
Rcpp::NumericVector hampel(Rcpp::NumericVector x, int window_length, double a=3.0, double k=1.4826, bool recentre=true) {
  int n = x.length();
  int shift = recentre ? floor(window_length / 2) : 0;
  double xbar;
  double mad;
  Rcpp::NumericVector window(window_length);
  Rcpp::NumericVector abs_diff(window_length);
  Rcpp::NumericVector s(n);
  if (n > 1) {
    for (int i = shift; i < n; i++) {
      if (i < (window_length - 1)) {
        ...
        window = x[Rcpp::Range(i-window_length+1, i)];
        xbar = median(window);
